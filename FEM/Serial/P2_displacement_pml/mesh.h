...
// 单个 P2 三角形单元（6 节点：3 顶点 + 3 边中点）
//   本地节点顺序（标准 P2 约定）：
//       0,1,2 = 三个顶点 (CCW)
//       3 = 边(0-1)中点, 4 = 边(1-2)中点, 5 = 边(2-0)中点
//   P2 形函数是二次，梯度线性（非常数）→ 装配用高斯求积，不预存梯度。
//   这里只存 6 个全局 node id、cell_id，以及顶点的 P1 常梯度 (dL_dx/dL_dz)——
//   因为 P2 形函数用面积坐标 L_i 表达，而 ∂L_i/∂x 是常数（= P1 梯度），
//   P2 梯度 = 二次多项式对 L 求导再乘常数 ∂L/∂x，装配时按高斯点现算。
struct Tri6 {
    int  nodes[6];
    int  cell_id;
    double A;          // |面积|
    double A_signed;
    double dL_dx[3];   // ∂L_i/∂x（面积坐标梯度，常数 = P1 顶点梯度）
    double dL_dz[3];
};
...
public:
    ...
    // P1 三角形连接表（-DUSE_P1_MESH 时填充；默认 P2 时为空）
    std::vector<Triangle> triangles;

    // P2 六节点三角形连接表（默认填充；-DUSE_P1_MESH 时为空）
    std::vector<Tri6> tris6;
    ...
    int n_tris6() const { return (int)tris6.size(); }
    int n_quads() const { return (int)quads.size(); }

private:
    void build_nodes();
    void build_triangles();
    void build_triangles_p2();
    void build_quads();
