...
## SESSION 12 (2026-07-07, 恢复 P1 版本 + 给 P1 声源加 3×3 高斯空间平滑压尾波)

### 用户诉求

把 backup_v8_P1_displacement_pml_20260707/ 的 P1 代码恢复到 Serial 顶层（替换当前 P2
版本），并按之前诊断（SESSION 9/10：尾波主因是单节点点源的宽带激发，含 λ<2dx 分量）
给 P1 声源做 **3×3** 空间平滑。

### 操作

1. `make clean` 后用 backup_v8_P1_displacement_pml_20260707/ 的 15 个源码 + Makefile
   覆盖顶层。CHANGELOG.md 在顶层、备份内没有，未被触碰。当前 P2 版本已另存于
   P2_displacement_pml/，不受影响。
2. 确认恢复后为纯 P1（config.h/mesh.h/assembler.cpp 无 Tri6/MX/Nvert/USE_P1_MESH）。

### Change 12.1  P1 声源 3×3 高斯空间平滑（assembler.cpp）

- 把矩张量源在源周围**节点**上做高斯平滑：对每个"子源节点"仍施加标准
  F_a=M0·∇N_a，按归一化高斯权重（∑w=1，保持总矩不变）叠加。展宽点源、削掉
  λ<2dx 分量 → 抑制 P1 无法干净传播的超短波长 → 压后期尾波。
- 开关：`-DSRC_SMOOTH_R=R`（窗口 (2R+1)²，**默认 1 = 3×3**）、
  `-DSRC_SMOOTH_SIGMA=σ`（默认 1.0）、R=0 退回集中点源。
- 只改 RHS 声源力系数，**不动 A**（A 仍对称 SPD，CIM 接口零影响）。

### 验证

- 编译无警告。
- **尾波下降**：200×200 均质无 PML 远场探针(100,90)：
  集中源 R=0 尾波 2.57% → 3×3 R=1 尾波 **1.53%**（降约 40%）。
  比 P2 的改善（SESSION 10：2.58%→2.27%，仅 12%）显著得多，印证尾波主因是点源
  宽带激发、源平滑才对症。
- **波前仍圆**：200×200 带 PML it800 diag/axis=1.037；源节点 txx=-1.3e-3（无尖峰）。
- 默认构建 = P1 + 位移二阶 + ADE-PML + 3×3 源平滑。

### 回滚

```bash
make CXXFLAGS+="-DSRC_SMOOTH_R=0"   # 退回集中点源（尾波回到 ~2.6%）
# 或整体恢复其它版本：cp backup_v8_P1_displacement_pml_20260707/* .（无平滑的原始P1）
```

---

## SESSION 13 (2026-07-07, 诊断"边界反射"：确认后期弥漫信号主体是 P1 频散尾而非反射；修好 ψ 抵消 bug)

### 用户现象与怀疑

用户用窄色标 (vmin/vmax=±1e-4) 看 txx 快照/视频，发现波传到边界后有部分"传回"，
怀疑声源平滑没治好后期频散、根因在边界处理。

### 诊断过程与结论

**(1) 确认当前 ADE-PML 的 ψ 记忆项完全失效**：
- 关 ψ (F_ψ=0) vs 开 ψ，it=1400 快照 `||on-off||=0` **逐位完全相同** → ψ 毫无贡献，
  PML 退化成纯 C+P 质量比例阻尼海绵层（不阻抗匹配）。
- ψ 失效根因（数值探针）：`||Kxx·u|| ≈ ||Kzz·u||`，而旧版 psi_xx/psi_zz 的方向因子
  `(dz-dx)`/`(dx-dz)` 符号相反 → `F_ψ ≈ dz·(Kxx·u - Kzz·u) ≈ 0`，两项抵消。
  这是 SESSION 8 unsplit 推导的**公式级错误**。

**(2) 修复 ψ（Change 13.1/13.2）**：按正确 unsplit CFS 重推——把刚度按**检验函数
导数方向**拆成 `K_dirx + K_dirz = K`（Ke_dx/Ke_dz，含对角+交叉项，仅算 RHS 不进 A），
两个独立记忆场 `ψ_x^{n+1}=E_x ψ_x^n+(1-E_x)(K_dirx·u)`、`ψ_z` 同构，
`F_ψ=ψ_x+ψ_z`（**直接相加，不做差**，消除抵消）。修后 ψ 从"完全无效"变为
"有 8–30% 影响"，但仍偏弱，非合格 <1% PML。

**(3) 关键：分离"边界反射" vs "P1 频散尾"（决定性）**：
- 对照 `NABS=30 (PML)` vs `NABS=0 (自由边界全反射)`，均质 200×200：
  - it=1700 内部残余：两者 = 2.96e-2，**100% 相同**
  - it=2600 内部残余：两者 = 1.00e-2，**逐位相同 (差异 0.0%)**
- 若内部残余是边界反射，自由边界(全反射)必然远强于 PML；两者逐位相同 →
  **内部弥漫信号根本不是边界反射，而是域内固有的 P1 频散尾波**（SESSION 9 已诊断，
  与 PML/边界无关）。
- 旁证：加厚/加强吸收层（NABS 30→40、RC 1e-4→1e-6）内部残余完全不变（都 1.3e-2）。

**结论**：用户看到的后期弥漫"传回"信号，主体是 **P1 一阶单元的频散尾波**，不是边界反射。
真正的边界反射回到内部更晚、量级被频散尾掩盖。当前 PML 吸收确实偏弱（≈自由边界），
但不是该现象的主因。声源平滑治不了频散尾是对的——因为那是单元阶数（P1）的固有频散，
需靠 P2/细网格解决（见 SESSION 10-11 的 P2 版本，频散尾更小）。

### 改动文件
- `assembler.cpp`：`build_system` 增加 Ke_dx/Ke_dz 方向分解装配到 K_xx/K_zz（含交叉项）；
  `pml_update_force` 改为正确 unsplit CFS（ψ_x/ψ_z 独立、F_ψ=ψ_x+ψ_z 不做差）。
- 不动：A 的对称正定、Newmark 主求解、CIM 接口、声源平滑、P1 网格。ψ 仍只进 RHS。

### 建议（留给后续）
- 若要**根治后期尾波**：用 P2 网格（`P2_displacement_pml/` 已有）或细网格，而非调 PML。
- 若要**PML 反射 <1%**：当前 unsplit ψ 仍偏弱，需继续打磨公式或改 split-field
  （但 split-field 破坏 A 对称正定、伤 CIM，权衡见 SESSION 对话）。当前 PML 反射
  被频散尾掩盖，暂不影响主用途。

### 回滚
```bash
# ψ 改动仅影响 RHS，关 PML 即回到纯位移：
make CXXFLAGS+="-DNABSDEF=0"
```

---

## SESSION 14 (2026-07-07, 修正边界反射：F_ψ 符号反了 + 加强吸收层)

### 用户纠正（关键）

用户用窄色标快照/mp4 指出：SESSION 13 我判断"后期弥漫是 P1 频散尾、非边界反射"是**错的**。
从快照明显看到波在底边/侧边反射回来（平行边界的强条带 ±5e-3），且旧版（session12）
吸收比 SESSION 13 改后**更好**。根因确是 PML 没吸收。（我此前用"内部残余"数值指标
误判，向用户道歉。）

### 诊断：两个 bug

**(1) F_ψ 符号反了**：能量对照 it=2500 ‖v‖²：
- 关 ψ（纯 C+P 海绵）= 6.43e-15
- SESSION 13 的 `-F_ψ` = 8.72e-15（**比海绵还差** → 符号错，ψ 在往回注能量）
- `+F_ψ` = 6.07e-15（最好）
- 推导核对：拉伸 1/s=1-d/(iω+d) → 内力=K·u-F_ψ，移到右端 b=f-Cv-(K·u-F_ψ)=…**+F_ψ**。

**(2) 吸收层太弱**：默认 NABS=30/RC=1e-4 反射仍大。加强后（100×100 对照）：
- NABS=25/RC=1e-4：内部反射残余 it1200 ≈ 4.6e-3
- NABS=45/RC=1e-8：≈ 1.8e-4（**25× 改善**）

### Change 14.1  main.cpp — F_ψ 符号修正
`b[i] = F - Cv - Ku + Fpsi`（原为 -Fpsi）。只改 RHS，不碰 A。

### Change 14.2  config.h — 加强吸收层默认值
`NABSDEF 30→40`，`RC_ABS 1e-4→1e-7`。

### 验证（200×200 均质 it=3450，对照用户原图）
- 用户原图边界反射条带 ±5e-3；修正后**全域 max=3.16e-4**（≈16× 小），
  边界带与内部同量级、无明显反射条带。
- 窄色标 (±1e-4) 下：主体反射消失，仅上方**角区**残留少量（角区 d_x,d_z 同时非零，
  PML 角区吸收本就最难）+ 中轴对称的 P1 频散尾。
- 反射量级整体从 ±5e-3 降到 ±1e-4（~50×）。
- **A 仍严格对称 rel=0.00e+00 (CIM-ready)**：符号/强度改动只碰 RHS 与阻尼，不碰 A。

### 遗留
- 角区残余可进一步压（角区专门处理 / 更厚层）。当前主体反射已解决。
- 中轴频散尾是 P1 固有（SESSION 9），需 P2/细网格，与本次边界修正正交。

### 回滚
```bash
make CXXFLAGS+="-DNABSDEF=30 -DRC_ABS=1e-4"   # 回到旧弱 PML（符号修正保留）
```
